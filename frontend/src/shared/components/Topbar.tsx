...
const farms = ['Main Farm', 'Greenhouse Complex', 'North Valley'];
  ...
  return (
    <header className="h-14 border-b border-border-default bg-surface-base flex items-center justify-between px-4 md:px-6">
      <div className="flex items-center gap-3">
        ...
        {/* Farm selector */}
        <select
          className="bg-transparent text-sm font-medium text-text-primary border-none focus:outline-none cursor-pointer"
          defaultValue="Main Farm"
        >
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2 pl-2 border-l border-border-default">
          ...
          <div className="w-7 h-7 rounded-full bg-accent/20 flex items-center justify-center text-xs font-bold text-accent">{user?.username?.charAt(0).toUpperCase() || '?'}</div>
          <div className="hidden sm:block">
            <span className="text-sm text-text-primary block leading-tight">{user?.username}</span>
            <span className="text-[10px] text-text-muted capitalize">{user?.role || 'user'}</span>
          </div>
          <button onClick={handleLogout} className="p-3 md:p-1.5 min-h-[44px] min-w-[44px] flex items-center justify-center text-text-muted hover:text-critical rounded-md hover:bg-surface-hover" title={t('user.logout')}>
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
